...
#$WORKSPACE_ROOT/venv/bin/bokeh serve --show live_gripper.py  --port 5011
...
CONTROLLER_STATES_TOPIC = '/mico_finger_controller/controller_state'
TRAJECTORY_TOPIC = '/mico_finger_controller/joint_trajectory'
JOINT_NAMES = [
...
class ROSJointDataCollector(Node):
    def __init__(self, joints_queue, trajectory_queue, joint_names):
        ...
        self.create_subscription(JointTrajectory, TRAJECTORY_TOPIC, self.trajectory_callback, 10)
    ...
    def _msgtime_to_nanoseconds(self, msg_time: MsgTime):
        ...
        if msg_time is not None:
            ...
            # No msg_time (e.g. no header.stamp)
    ...
    def joint_state_callback(self, msg):
        ...
        if joint_data:
            self.queue.put(('joints', time, joint_data))
    ...
    def controller_state_callback(self, msg):
        ...
        for name, pos, vel, ref_pos, ref_vel, cmd_vel in zip(msg.joint_names,
                                                    ...
                                                    msg.output.velocities,
                                                    ):  # Assuming velocity interface
            if name in self.joint_names:
                joint_data[name] = {
                    'pos': pos,
                    'vel': vel,
        ...
        if joint_data:
            self.queue.put(('control', time, joint_data))
            self.last_update = time
...
for jnt, name in enumerate(JOINT_NAMES):
    ctrl_source_data[f'pos_{jnt}'] = []
    ctrl_source_data[f'vel_{jnt}'] = []
    ctrl_source_data[f'ref_pos_{jnt}'] = []
    ctrl_source_data[f'ref_vel_{jnt}'] = []
    ctrl_source_data[f'cmd_vel_{jnt}'] = []

    joint_source_data[f'pos_{jnt}'] = []
    joint_source_data[f'vel_{jnt}'] = []
    joint_source_data[f'effort_{jnt}'] = []

    traj_source_data[f'pos_cmd_{jnt}'] = []
    traj_source_data[f'vel_cmd_{jnt}'] = []


ctrl_sources = ColumnDataSource(data=ctrl_source_data)
...
for jnt, name in enumerate(JOINT_NAMES):
    ...
    p_pos.line('t', f'ref_pos_{jnt}', source=ctrl_sources, color='red', legend_label='Reference')
    ...
    future_line_vel = p_vel.line('tt', f'vel_cmd_{jnt}', source=traj_sources,
                                color='maroon', line_dash='dashed', legend_label='Trajectory')
    p_vel.line('t', f'cmd_vel_{jnt}', source=ctrl_sources , color='cyan', legend_label='Command')
    p_vel.line('t', f'ref_vel_{jnt}', source=ctrl_sources, color='red', legend_label='Reference')
    ...
    p_eff = figure(title=f"{name} Effort", x_axis_label="Time (s)", y_axis_label="Effort (N·m)",
                    y_range=(-1, 1),
                    tools="pan,wheel_zoom,box_zoom,reset",
                    active_scroll="wheel_zoom", sizing_mode="stretch_both")
    p_eff.line('t', f'effort_{jnt}', source=joint_sources , color='cyan', legend_label='Command')
    p_vel.legend.click_policy = 'hide'
    p_vel.legend.background_fill_alpha = 0.3  # Semi-transparent
    eff_plots.append(p_eff)
...
plot_refs = []
for i, plt in enumerate(posn_plots):
    plot_refs += [plt, vel_plots[i], eff_plots[i]]

grid = gridplot(plot_refs, ncols=3, sizing_mode="stretch_both")
...
def reset_all():
    for p in posn_plots:
        ...
        p.y_range = DataRange1d(start=-np.pi, end=np.pi)
    ...
    for p in eff_plots:
        p.x_range = DataRange1d()
        p.y_range = DataRange1d(start=-1., end=1.)
        # Reset all legend-hidden glyphs to visible
        for r in p.renderers:
            if hasattr(r, 'visible'):
                r.visible = True
...
def update():
    ...
    for jnt in range(len(JOINT_NAMES)):
        ctrl_stream_dict[f'pos_{jnt}'] = []
        ctrl_stream_dict[f'vel_{jnt}'] = []
        ctrl_stream_dict[f'ref_pos_{jnt}'] = []
        ctrl_stream_dict[f'ref_vel_{jnt}'] = []
        ctrl_stream_dict[f'cmd_vel_{jnt}'] = []

    joints_stream_dict = {'t': []}
    for jnt in range(len(JOINT_NAMES)):
        joints_stream_dict[f'pos_{jnt}'] = []
        joints_stream_dict[f'vel_{jnt}'] = []
        joints_stream_dict[f'effort_{jnt}'] = []

    while not joints_data_queue.empty():
        msg = joints_data_queue.get()
        if msg[0] == 'control':
            _, t, joint_data = msg
            ctrl_stream_dict['t'].append(t * 1.e-9)

            for jnt, name in enumerate(JOINT_NAMES):
                ctrl_stream_dict[f'pos_{jnt}'].append(joint_data[name]['pos'])
                ctrl_stream_dict[f'vel_{jnt}'].append(joint_data[name]['vel'])
                ctrl_stream_dict[f'ref_pos_{jnt}'].append(joint_data[name]['ref_pos'])
                ctrl_stream_dict[f'ref_vel_{jnt}'].append(joint_data[name]['ref_vel'])
                ctrl_stream_dict[f'cmd_vel_{jnt}'].append(joint_data[name]['cmd_vel'])
        elif msg[0] == 'joints':
            _, t, joint_data = msg
            joints_stream_dict['t'].append(t * 1.e-9)

            for jnt, name in enumerate(JOINT_NAMES):
                joints_stream_dict[f'pos_{jnt}'].append(joint_data[name]['pos'])
                joints_stream_dict[f'vel_{jnt}'].append(joint_data[name]['vel'])
                joints_stream_dict[f'effort_{jnt}'].append(joint_data[name]['effort'])

    # Only call stream once per update
    if ctrl_stream_dict['t']:
        ctrl_sources.stream(ctrl_stream_dict, rollover=ROLLOVER_WINDOW)

    if joints_stream_dict['t']:
        joint_sources.stream(joints_stream_dict, rollover=ROLLOVER_WINDOW)

    if len(joint_sources.data['t']) > 0:
        latest_t = joint_sources.data['t'][-1]
    else:
        latest_t = 0
    if len(ctrl_sources.data['t']) > 0 and latest_t < ctrl_sources.data['t'][-1]:
        latest_t = ctrl_sources.data['t'][-1]
