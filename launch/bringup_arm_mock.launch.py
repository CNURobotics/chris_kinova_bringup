...
def generate_launch_description(kinova_arm_name='m1n6s200'):
    ...
    default_arguments.append(
        LogInfo(
            msg=PythonExpression(['"Using description package: ', LaunchConfiguration("description_package"), '"']),
        )
    )
    default_arguments.append(
        LogInfo(
            msg=PythonExpression(['"Using default description_file: ', LaunchConfiguration("kinova_arm"), '.urdf.xacro"']),
    ...
    default_arguments.append(
        SetLaunchConfiguration(
            ...
            value=PythonExpression(['"', LaunchConfiguration("kinova_arm"), '.urdf.xacro"']),
            condition=IfCondition(
                EqualsSubstitution(LaunchConfiguration("description_file"), "None")
            )
        )
